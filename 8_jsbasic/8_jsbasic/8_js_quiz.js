...
// (선언과 호출 두개 다 작성할 것!)
let hello = 2 // function으로 함수 선언 한건만 호출할 수 있나요?? 
console.log(hello);
...
let box1 = document.querySelectorAll('span');
// box1.classList.add('hello');
console.log(box1);
// box1.addEventListener('hello');
box1.forEach( function (order, index) { // []배열 객체에서 꺼내쓰기 위해 forEach 함수를 쓴다
  console.log(order, index);
  order.classList.add('hello');
});
...
if (boxEl.classList.contains('active')) { //변수에 안담고 바로 씀
  console.log('포함됨!');
}else {
  console.log('포함 안됨!');
}