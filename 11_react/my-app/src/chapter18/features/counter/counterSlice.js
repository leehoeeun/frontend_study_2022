...
const counterSlice = createSlice({
  ...
  initialState,  // 초기값의 state를 정할 수 있음  ==> (생략 가능) initialState: initialState,
  reducers: {
    // reducers 함수란? state를 변경하는 함수이다.
    // 현재 state와 action 객체를 파라미터로 받아오고 필요한 경우 상태를 업데이트 하고 개 상태를 반환하는 함수
    increment: (state)=>{   // 첫번째 파라미터: 현재 state를 받아옴
      state.value += 1;  // 배열 또는 객체의 경우에도 직접 수정하는 형태로 작성 가능!!
      // How? Immer 라이브러리가 내장되어 불변성 관리를 해줌 (복사본을 만들어서 새로운 객체, 배열로 다시 뿌려줌)
      // 실제로 상태를 직접 변경하지 않고 내부적으로 state 복사본을 만들어서 개를 변경하고 새로운 상태를 반환함
    },
    decrement: (state) => {
      state.value -= 1;
    },
    incrementByAmount: (state, action) => { // 두번째 파라미터: action 객체 = { type: 발생한 액션의 타입, payload: 화물, 전달 데이터, 데이터를 담아서 보낼 수 있다 }
      console.log(action);
...
console.log(counterSlice); // TODO : counterSlice여기에 어떤 속성값이 있는지 확인해보시길
