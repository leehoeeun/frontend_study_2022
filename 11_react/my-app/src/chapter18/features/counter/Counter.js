...
function Counter(props) {
  ...
  // const count = useSelector((state) => state.counter.value);  // state.counter.value 가 아니라 state만 적으면(return하면) 전역 state 전부 가져옴
    ...
    <div>
      <div>
        <button 
          ...
          onClick={() => dispatch(decrement())}   // dispatch()함수를 쓰면서 안에 action을 담아서 보내야 된다
        >
          -1 감소
        </button>
        <span>{count}</span>
